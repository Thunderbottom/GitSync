...
parser.add_argument('--no-skip-forks','-ns', help='Skips if the current repository is a fork', action="store_true")
parser.add_argument('--directory', '-d', help='Set location for sync. If not set, cwd is used')
parser.add_argument('---lang-dir', '-ld', help='Segregate git project folders by language (May or may not work with existing projects)', action="store_true")
parser.add_argument('--commit-all', '-c', help='Commit everything to the current git repository', action="store_true")
...
# Clone repository TODO:Segregate by language folders
...
def gitSync(repoURL, workDir):
    ...
    if commitAll:
        ...
        if not commitMessage:
            commitMessage = input('No commit message found for repository', repoURL,'\nPlease enter a commit message: ')
        os.system('git add --all && git commit -m "{}" &> /dev/null'.format(commitMessage))
...
def gitSyncFork(repoURL, workDir):
    ...
    os.system('git -C ' + workDir + '/ remote add upstream ' + parentURL + '&> /dev/null')
...
def syncRepo(repoURL, language, path, isFork):
    ...
    if cloneAll:    # Since no directory existed in previous iteration, we'll clone all the repos
        ...
        clone = input('No directories exist. Do you want to clone all the repositories? (Yes) ')
        if not clone:
            clone = 'yes'
        if clone.lower() == 'yes':
            cloneAll = 1
        ...
        for directory in os.listdir(workDir): # Check every file in the directory
            if os.path.isdir(os.path.join(workDir, directory)): # Check if the current value of directory is a directory
                ifGit = checkGit(os.path.join(workDir, directory))
                if ifGit:   # Current directory is a git directory
                    getremoteURL = subprocess.Popen('git -C ' + os.path.join(workDir, directory) + ' config --get remote.origin.url', stdout=subprocess.PIPE, shell=True)
                    ...
                    if repoURL == remoteURL.strip('\n'):
                        ...
                        notFound = 1
                # else:
                #     print (directory, "is not a git directory, skipping")
        if notFound:
            if skipForks:
                if not isFork:
                    clone = input("Git repository for " + repoURL + " not found. Do you want to clone the repo? (Yes) ")
                    if not clone:
                        clone = "yes"
                    if clone.lower() =='yes':
                        cloneRepo(repoURL, workDir)
...
def main():
    ...
    try:
        getReq = requests.get("https://api.github.com/users/{}/repos".format(user))
    ...
    for getURL in getJSON:
        syncRepo(getURL["clone_url"], getURL["language"], currDir, getURL["fork"])
    print('Done.')
    sys.exit()
